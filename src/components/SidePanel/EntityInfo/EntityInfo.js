        ...
        if (this.context.user && this.context.user.user_name === this.props.user_name) {
            buttons = (
                <div className='display-buttons'>
                    <button className='edit' onClick={e => this.context.methods.setMode('edit')}>Edit</button>
                    <button className='delete' onClick={e => this.handleDelete()}>Delete</button>
                </div>
            )
        }
                ...
                <div className='name-and-user'>
                    ...
                    {this.props.type === 'waypoint' ? (
                        <div className='latitude info-box'>
                            <h6>Latitude:</h6>
                            <p>{(Math.round(CesiumMath.toDegrees(this.props.position.latitude) * 100)/100).toFixed(2)}</p>
                        </div>
                    ) : ''}
                    {this.props.type === 'waypoint' ? (
                        <div className='longitude info-box'>
                            <h6>Longitude:</h6>
                            <p>{(Math.round(CesiumMath.toDegrees(this.props.position.longitude) * 100)/100).toFixed(2)}</p>
                        </div>
                    ) : ''}
                    {this.props.type === 'route' ? (
                        <div className='distance info-box'>
                            <h6>Distance:</h6>
                            <p>{(Math.round(this.props.distance*100)/100).toFixed(2)} miles</p>
                        </div>
                    ) : ''}
